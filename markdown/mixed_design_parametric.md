...
Since the treatment and the control groups are independent, an estimate of the sampling variance of $d_{PPC-change}$ is given by the sum:

$$ \widehat{Var}(d_{PPC-change}) = \widehat{Var}(d_{z,\,a} - d_{z,\,b}) = \widehat{Var}(d_{z,\,a}) + \widehat{Var}(d_{z,\,b}) $$

Viechtbauer (2007) compared the performance of confidence interval CI (CI) procedures based on various approximations of the sampling variance of $d_z$. Based on this comparison Viechtbauer recommends using the following formula to estimate the sampling variance of $d_z$:
...
An approximate large-sample $1 - \alpha$ CI is implemented similarly as the CI for $d_{PPC-Change}$ with  $\widehat{Var}(g_{PPC-change})$ replacing $\widehat{Var}(d_{PPC-change})$ in the formula for computing the confidence limits for $d_{PPC-Change}$:

$$\widehat{Var}(g_{PPC-change} = J(\nu_a)^2\hat{\sigma}^2_{d_{z,\,a}} + J(\nu_b)^2\hat{\sigma}^2_{d_{z,\,b}} $$
...
Viechtbauer (2007) compared the performance of CI procedures based on various formulas for the approximate sampling variance of $d_{G,\,pre} = \frac{\bar{X}_{post} - \bar{X}_{pre}}{s_{pre}}$. While none of the methods was superior in all analysed aspects and with in fact all of the procedures yielding coverage probabilities below the nominal $1 - \alpha$ level, the author generally concludes that the following formula for the sampling variance of $d_{G,\,pre}$ was the most accurate:
...
Becker (1988) termed the population ES given by $Eq.(PPC)$ $\Delta$ and its estimate $\hat{\Delta}$ and Morris (2008) called it $d_{PPC1}$. Since we are referring to the bias-corrected ESs with a $g$, we have opted for $g$ instead of $d$. Additionally, we have put information about the study design (PPC) and the standardiser (pretest standard deviations) in the subscript.
...
$$\widehat{Var}(g_{PPC,\,pre} = \hat{\sigma}^2_{g_{G,\,pre,\,a}} + \hat{\sigma}^2_{g_{G,\,pre,\,b}} $$
...
$$r_p = \frac{e^{2z_p} - 1}{1 + e^{2z_p}} Eq.(PPC)$$
...
Morris (2008) termed this estimator $d_{PPC2}$. Since we are referring to the bias-corrected ESs with a $g$, we have opted for $g$ instead of $d$. Additionally, we have put information about the study design (PPC) and the standardiser (pooled pretest standard deviations) in the subscript.  
...
While Morris (2008) did not provide a closed form formula for the sampling distribution of $d_{PPC,\,pooled-pre-post}$ for reasons mentioned above, the author scrutinised the empirical sampling distribution of the ES using Monte Carlo simulations. When data were simulated from bivariate normal distributions with equal variances across times of measurement and populations as well as with equal population pre-posttest correlations, the empirical sampling distribution of $d_{PPC,\,pooled-pre-post}$ was generally the smallest of the three ESs Morris (2008) compared. However, it should be noted that the sampling variance of $d_{PPC,\,pooled-pre-post}$ consistently proved to be a marginal improvement over the sampling variance of $d_{PPC,\,pooled-pre}$ (Morris, 2008).  

As noted above, the formula for the sampling variance of $d_{PPC,\,pooled-pre-post}$ is derived under the assumption of homogeneous variances across populations and times of measurement (amongst under assumptions). Should the posttest variance in one or both examined populations be larger than the pretest variance - for example due to some test subjects improving more strongly as a result of a treatment than others - then the given formula for the sampling variance and consequently of the CI of $d_{PPC,\,pooled-pre-post}$ is inaccurate. Further, the Monte Carlo simulations of Morris (2008) have shown that sampling under heteroscedasticity increases the empirical sampling variance of $d_{PPC,\,pooled-pre-post}$  (much like the sampling variance of the other analysed ES estimates) as compared to sampling under homoscedasticity.  
...
An $1 - \alpha$ percentile bootstrap CI is implemented, which should give a better estimate of the magnitude of the true sampling variance of $d_{PPC,\,pooled-pre-post}$ under heteroscedasticity and thus provide a more accurate CI if one is still interested in employing $d_{PPC,\,pooled-pre-post}$ as an ES.
...
with $\nu= \frac{2(n_a + n_b - 2)}{1 + r_p^2}$ denoting the degrees of freedom (for the computation of $r_p$ see equations $Eq.(PPC)$ through $Eq.(PPC)$) and $J(\nu)$ being given by $Eq.(PPC)$. This ES also estimates $\delta_{PPC,\,pooled-pre-post}$.

Morris (2008) termed this estimator $d_{PPC3}$. Since we are referring to the bias-corrected ESs with a $g$, we have opted for $g$ instead of $d$. Additionally, we have put information about the study design (PPC) and the standardiser (pooled pretest and posttest standard deviations) in the subscript.  

Same as for $d_{PPC,\,pooled-pre-post}$ a delta-method approximation suggested by Pustejovsky (2016) is implemented which importantly assumes homogeneity of variances across times of measurements and populations (for furhter assumptions see the introduction to the difference-focused PPC ESs). The estimate of the sampling variance is given by:
