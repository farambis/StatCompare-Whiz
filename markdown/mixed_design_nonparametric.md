...
$$\hat{\delta}_{PPC-Change} = \Phi^{-1}(\hat{p}_{gain,\,a}) - \Phi^{-1}(\hat{p}_{gain,\,b}) = \hat{\delta}_{z,\,a} - \hat{\delta}_{z,\,b}$$  

where $\hat{p}_{gain,\,a}$ and $\hat{p}_{gain,\,b}$ are given by:

$$\hat{p}_{gain,\,a} = \frac{x_{post,\,a} > x_{pre,\,a}}{n}$$
$$\hat{p}_{gain,\,b} = \frac{x_{post,\,b} > x_{pre,\,b}}{n}$$

Thus, $hat{p}_{gain,\,a}$ and $\hat{p}_{gain,\,b}$ are the proportions of positive change scores ($x_{post} - x_{pre}$) in each group. If ties between pre- and posttest scores are present, each tie is counted as half an observation in each direction. This estimator is the difference between the $\hat{p}_{gain,\,a}$- and the $hat{p}_{gain,\,b}$-quantile of the standard normal distribution. Much like $d_{PPC-Change}$ simplifies to the difference between Cohen's $d_z$ for groups a and b, so too does it's nonparametric counterpart simplify to the difference between the nonparametric versions of Cohen's $d_z$ for groups a and b: $\hat{\delta}_{z,\,a}$ and $\hat{\delta}_{z,\,b}$.  

The reason $\hat{\delta}_{PPC-Change}$ can be considered a nonparametric version of $d_{PPC-Change}$ is owed to the fact that when scores are normally distributed, then $\hat{\delta}_{PPC-Change}$ is an estimator of the population effect size given by $\delta_{PPC-Change} = \frac{\mu_{post,\,a} - \mu_{pre,\,a}}{\sigma_{D,\,a}} - \frac{\mu_{post,\,b} - \mu_{pre,\,b}}{\sigma_{D,\,b}}$ (see the mixed design parametric documentation page entry on  $d_{PPC-Change}$ for further details on this population effect).  

It should be noted that Hedges and Olkin (1985) denoted this estimator as $\hat{\delta}_3$. The subscript has been changed from $3$ to $PPC-Change$ in order to match the subscript used for this estimator with the subscript used for the corresponding parametric estimator $d_{PPC-Change}$.  

When n is small $\hat{p}_{gain,\,a|b}$ might be equal to 0 or 1. To avoid consequently extreme effect sizes  $\hat{p}_{gain,\,a|b}$ is set to $\frac{1}{n_{a|b}+1}$  or  $\frac{n_{a|b}}{n_{a|b}+1}$ respectively in such cases.  
...
$$\hat{\delta}_{PPC,\,pre} = \phi^{-1}(\hat{p}_{pre,\,a}) - \phi^{-1}(\hat{p}_{pre,\,b}) = \hat{\delta}_{pre,\,a} - \hat{\delta}_{pre,\,b}$$

where $\hat{p}_{pre,\,a}$ and $\hat{p}_{pre,\,b}$ are given by:

$$\hat{p}_{pre,\,a} = \frac{x_{pre,\,a} < median(x_{post,\,a})}{n}$$
$$\hat{p}_{pre,\,b} = \frac{x_{pre,\,b} < median(x_{post,\,b})}{n}$$

Thus $\hat{p}_{pre,\,a}$ and $\hat{p}_{pre,\,b}$ are the proporion of pretest scores of a group that are lower than the median of the posttest scores of the respective group. Consequently, $\hat{\delta}_{PPC,\,pre}$ is the difference between the $\hat{p}_{pre,\,a}$- and the $hat{p}_{pre,\,b}$-quantile of the standard normal distribution. Much like $d_{PPC,\,pre}$ simplifies to the difference between dependent groups Glass' $d_{G,\,pre}$ for groups a and b, so too does it's nonparametric counterpart simplify to the difference between the nonparametric versions of dependent groups Glass' $d_{G,\,pre}$ for groups a and b: $\hat{\delta}_{pre,\,a}$ and $\hat{\delta}_{pre,\,b}$.  

The reason $\hat{\delta}_{PPC,\,pre}$ can be considered a nonparametric version of $d_{PPC,\,pre}$ is owed to the fact that when scores are normally distributed, then $\hat{\delta}_{PPC,\,pre}$ is an estimator of the population effect size given by $\delta_{PPC,\,pre} = \frac{\mu_{post,\,a} - \mu_{pre,\,a}}{\sigma_{pre,\,a}} - \frac{\mu_{post,\,b} - \mu_{pre,\,b}}{\sigma_{pre,\,b}}$ (see the mixed design parametric documentation page entry on  $d_{PPC,\,pre}$ for further details on this population effect).  

It should be noted that Hedges and Olkin (1985) denoted this estimator as $\hat{\delta}_1$. The subscript has been changed from $1$ to $PPC,\,pre$ in order to match the subscript used for this estimator with the subscript used for the corresponding parametric estimator $d_{PPC,\,pre}$.  

When n is small $\hat{p}_{pre,\,a|b}$ might be equal to 0 or 1. To avoid consequently extreme effect sizes  $\hat{p}_{pre,\,a|b}$ is set to $\frac{1}{n_{a|b} + 1}$  or  $\frac{n_{a|b}}{n_{a|b} + 1}$ respectively in such cases.  
...
$$\hat{\delta}_{PPC,\,post} = \phi^{-1}(\hat{p}_{post,\,a}) - \phi^{-1}(\hat{p}_{post,\,b}) = \hat{\delta}_{post,\,a} - \hat{\delta}_{post,\,b}$$

where $\hat{p}_{post,\,a}$ and $\hat{p}_{post,\,b}$ are given by:

$$\hat{p}_{post,\,a} = \frac{x_{post,\,a} > median(x_{pre,\,a})}{n}$$
...
When n is small $\hat{p}_{post,\,a|b}$ might be equal to 0 or 1. To avoid consequently extreme effect sizes  $\hat{p}_{post,\,a|b}$ is set to $\frac{1}{n_{a|b} + 1}$  or  $\frac{n_{a|b}}{n_{a|b} + 1}$ respectively in such cases.  
